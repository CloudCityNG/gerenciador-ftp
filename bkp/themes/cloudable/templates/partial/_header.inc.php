    ...
    <div class="page-container horizontal-menu with-sidebar fit-logo-with-sidebar <?php echo($Auth->loggedIn() == true)?'logged-in':'logged-out'; ?>">	
        ...
        <div class="sidebar-menu fixed">
			...
			<?php if (UserPeer::getAllowedToUpload() == true): ?>
				<div class="sidebar-mobile-upload visible-xs"><a href="#" onClick="uploadFiles(); return false;"><?php echo t('upload_account', 'Envio'); ?>&nbsp;&nbsp;<span class="glyphicon glyphicon-cloud-upload"></span></a> </div>
			<?php endif; ?>
                ...
                <div class="navbar-form navbar-form-sm navbar-left shift" ui-shift="prependTo" data-target=".navbar-collapse" role="search">
                    <div class="form-group">
                        <div class="input-group" id="top-search">
                            <input type="text" value="<?php echo isset($_REQUEST['t'])?validation::safeOutputToScreen($_REQUEST['t']):''; ?>" class="form-control input-sm bg-light no-border rounded padder typeahead" placeholder="<?php echo addslashes(t('account_header_search_your_files', 'Pesquisando seus arquivos...')); ?>" onKeyUp="handleTopSearch(event, this); return false;" id="searchInput">
                            <span class="input-group-btn">
                                <button type="submit" class="btn btn-sm bg-light rounded" onClick="handleTopSearch(null, $('#searchInput')); return false;" title="" data-original-title="<?php echo t('filter', 'Filtrar'); ?>" data-placement="bottom" data-toggle="tooltip"><i class="entypo-search"></i></button>
                                <button type="submit" class="btn btn-sm bg-light rounded" onClick="showFilterModal(); return false;" title="" data-original-title="<?php echo t('advanced_search', 'Pesquisa avançada'); ?>" data-placement="bottom" data-toggle="tooltip"><i class="entypo-cog"></i></button>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <?php if ($Auth->loggedIn()): ?>
						...
						<li class="root-level responsive-Hide">
                            <a href="<?php echo ADMIN_WEB_ROOT; ?>/" target="_blank">
                                <span class="badge badge-danger badge-roundless"><?php echo strtoupper(t('admin_user', 'Usuário administrador')); ?></span>
                            </a>
                        </li>
						<?php
						}
                        ...
                        <li class="dropdown account-nav-icon">
                            ...
                            <ul class="dropdown-menu">
                                <?php
                                $label = UCWords(t('unlimited', 'ilimitado'));
                                ...
                                <?php if ($totalFileStorage > 0): ?>
                                                <span><span id="totalActiveFileSize"><?php echo validation::safeOutputToScreen(coreFunctions::formatSize($totalActiveFileSize)); ?></span> <?php echo t("of", "of"); ?> <?php echo validation::safeOutputToScreen(coreFunctions::formatSize($totalFileStorage)); ?> <?php echo t("used", "usado"); ?></span>
                                <?php else: ?>
                                                <span><span id="totalActiveFileSize"><?php echo validation::safeOutputToScreen(coreFunctions::formatSize($totalActiveFileSize)); ?></span> <?php echo t("of", "of"); ?> <?php echo UCWords(t('unlimited', 'ilimitado')); ?></span>
                                <?php endif; ?>
                                        </p>
                                    </div>
								...
								<?php if($packageId == 20): ?>
								<li>
                                    <a href="<?php echo ADMIN_WEB_ROOT; ?>/" target="_blank"></i> <i class="entypo-users"></i><?php echo t('admin_area_link', 'Área do Administrador'); ?></a>
                                </li>
                                <li class="divider"></li>
								<?php endif; ?>
                                <li>
                                    <a href="<?php echo coreFunctions::getCoreSitePath() . '/logout.' . SITE_CONFIG_PAGE_EXTENSION; ?>"> <i class="entypo-logout"></i><?php echo t('file_manager_logout', 'Sair do Sistema'); ?></a>
                                </li>
                            </ul>
                            <!-- / dropdown -->
                        </li>
