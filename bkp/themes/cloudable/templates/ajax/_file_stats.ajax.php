...
if (!$file)
{
    ...
    die('Não foi possível carregar o arquivo.');
}

// make sure user is permitted to view stats
if ($file->canViewStats() == false)
{
	die(t("stats_error_file_statistics_are_private", "As estatísticas desse arquivo não são visíveis publicamente."));
}
...
<div class="modal-header">
        ...
        <h4 class="modal-title"><?php echo validation::safeOutputToScreen($file->originalFilename).' '.t("stats_title", "Estatisticas"); ?>. (<?php echo validation::safeOutputToScreen($file->visits).' '.t("stats_downloads", "Downloads"); ?>)</h4>
    </div>
