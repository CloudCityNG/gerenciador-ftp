...
if ((int) $_REQUEST['submitme'])
{
    ...
    {
        notification::setError(t("please_enter_the_foldername", "Por favor entre com o nome da Pasta"));
    }
    ...
    else
    {
        ...
        if ($rs)
        {
            if (COUNT($rs))
            {
                notification::setError(t("already_an_folder_with_that_name", "Você já possui uma pasta com esse nome, use outra"));
            }
        }
    }
    ...
    if (!notification::isErrors())
    {
        ...
        if($editFolderId !== null)
        {
            ...
            {
                // success
                notification::setSuccess(t("folder_updated", "Pasta atualizada."));
            }
            else
            {
                notification::setError(t("problem_updating_folder", "Ocorreu um problema ao atualizar a pasta, tente novamente mais tarde."));
            }
        }
        // add folder
        else
        {
            ...
            {
                // success
                notification::setSuccess(t("folder_created", "Pasta criada."));
                $editFolderId = $db->insertId();
            }
            else
            {
                notification::setError(t("problem_adding_folder", "Ocorreu um problema ao adicionar a pasta, tente novamente mais tarde."));
            }
        }
...
$returnJson['msg'] = t("problem_updating_folder", "Ocorreu um problema ao atualizar a pasta, tente novamente mais tarde.");
if (notification::isErrors())
{
    // error
    $returnJson['success'] = false;
