...
if (isset($_REQUEST['u']))
{
    ...
    if (!$file)
    {
		...
		$returnJson = array();
		$returnJson['html'] = 'Arquivo não encontrado.';
    ...
    if (($file->userId != $Auth->id) && ($Auth->level_id < 10))
    {
		...
		if(coreFunctions::getOverallPublicStatus($file->userId, $file->folderId, $file->id) == false)
		{
			...
			$returnJson['page_url'] = '';
			$returnJson['javascript'] = 'showErrorNotification("'.str_replace("\"", "'", UCWords(t('error', 'Error'))).'", "'.str_replace("\"", "'", t('file_is_not_publicly_shared_please_contact', 'O arquivo não é compartilhado publicamente. Entre em contato com o proprietário e solicite que atualizem as configurações de privacidade.')).'");';
			echo json_encode($returnJson);
			exit;
		}
		...
		<div class="row">
			...
			<div class="col-md-3">
						...
						<tbody>
							<tr>
								<td class="view-file-details-first-row">
									<?php echo UCWords(t('uploaded', 'carregado')); ?>:
								</td>
							...
							<tr>
								<td class="view-file-details-first-row">
									<?php echo UCWords(t('filesize', 'tamanho do arquivo')); ?>:
								</td>
								<td class="responsiveTable">
							...
							<tr>
								<td class="view-file-details-first-row">
									<?php echo UCWords(t('sharing', 'Compartilhando')); ?>:
								</td>
								<td class="responsiveTable">
									<?php echo ($isPublic == true) ? '<i class="entypo-lock-open"></i>' : '<i class="entypo-lock"></i>'; ?>
									<?php echo ($isPublic == true) ? t('public_file', 'Arquivo público - Pode ser acessado diretamente por qualquer pessoa que conheça a URL do arquivo.') : t('private_file', 'Arquivo privado - Disponível somente na sua conta, ou através de uma URL de compartilhamento gerada.'); ?>
								</td>
							</tr>
							<?php endif; ?>
				...
				if($userOwnsFile == true)
				{
					if ($file->statusId == 1)
					{
						$links[] = '<button type="button" class="btn btn-default" data-dismiss="modal" onClick="showEditFileForm('.(int)$file->id.'); return false;" title="" data-original-title="'.addslashes(UCWords(t('account_file_details_edit_file', 'Editar Arquivo'))).'" data-placement="bottom" data-toggle="tooltip"><i class="entypo-pencil"></i></button>';
						$links[] = '<button type="button" class="btn btn-default" data-dismiss="modal" onClick="deleteFile('.(int)$file->id.', function() {loadImages('.((int)$file->folderId?$file->folderId:'-1').');}); return false;" title="" data-original-title="'.addslashes(UCWords(t('account_file_details_delete_file', 'Deletar Arquivo'))).'" data-placement="bottom" data-toggle="tooltip"><i class="entypo-trash"></i></button>';
					}
								...
								<form action="<?php echo WEB_ROOT; ?>/ajax/_account_file_details_send_email.process.ajax.php" autocomplete="off">
									<div class="col-md-12">
										<div class="form-group">
											<p><?php echo t('account_file_details_intro_user_the_form_below_send_email', 'Use o formulário abaixo para compartilhar este arquivo por e-mail. O destinatário receberá um link para baixar o arquivo.') ; ?></p>
											<?php if($isPublic == false): ?>
												<div class="alert alert-danger"><?php echo t('account_file_details_folder_not_publicly_shared', 'Este arquivo não é compartilhado publicamente. Você precisará torná-lo público antes que o destinatário possa vê-lo.') ; ?></div>
											<?php endif; ?>
										</div>

										<div class="form-group">
											<label class="control-label" for="shareRecipientName"><?php echo UCWords(t("recipient_name", "Nome completo do destinatário")); ?>:</label>
											<input type="text" id="shareRecipientName" name="shareRecipientName" class="form-control"/>
										</div>
						
										<div class="form-group">
											<label class="control-label" for="shareEmailAddress"><?php echo UCWords(t("recipient_email_address", "endereço de email do destinatário")); ?>:</label>
											<input type="text" id="shareEmailAddress" name="shareEmailAddress" class="form-control"/>
										</div>
						
										<div class="form-group">
											<label class="control-label" for="shareExtraMessage"><?php echo UCWords(t("extra_message", "mensagem extra (opcional)")); ?>:</label>
										...
										<div class="form-group">
											...
											<button type="button" class="btn btn-info" onClick="processAjaxForm(this, function() { $('#shareRecipientName').val(''); $('#shareEmailAddress').val(''); $('#shareExtraMessage').val(''); }); return false;"><?php echo UCWords(t("send_email", "enviar email")); ?> <i class="entypo-mail"></i></button>
										</div>
									</div>
								</form>
							</div>
